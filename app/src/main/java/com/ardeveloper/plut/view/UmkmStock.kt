...
class UmkmStock : BaseActivity(),UmkmAdapter.UmkmAdapterListener, UmkmAdapter.deleteListener {
    ...
    override fun onContactSelected(umkm: UMKM?) {
        val intent = Intent(this@UmkmStock, ProdukUmkm::class.java)
            ...
            apiInterface.hapusUmkm(umkm!!.kodeUmkm).enqueue(object : Callback<ResponseBody>{
                override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                    if (response.isSuccessful){
                        Toasty.success(this@UmkmStock,"UMKM Berhasil Dihapus").show()
                        getData()
                    }else{
                        Toasty.error(this@UmkmStock,"Error "+response.code()).show()
                    }
                }

                override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                    Toasty.error(this@UmkmStock,"Error "+t.message).show()
                }
